...
// Fetches recent head-to-head meetings and each side's recent form from a
// single Forebet match-prediction page (the URL captured per row by
// forebet.js's extractRows, e.g. forebet.com/en/predictions/<slug>).
//
...
// tune parseH2H/parseForm against it — the same workflow already used to
// verify every other scraper in this repo (see README "Debugging").
//
// Both parsers are structural-selector-first with a text-anchored
...
async function fetchForebetMatchInfo(url) {
  ...
  if (!h2h.length && !homeForm.length && !awayForm.length) return null;
  return { h2h, homeForm, awayForm, sourceUrl: url, fetchedAtISO: new Date().toISOString() };
}

module.exports = { fetchForebetMatchInfo, parseH2H, parseForm };
